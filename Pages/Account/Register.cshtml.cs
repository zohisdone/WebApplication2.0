...
namespace WebApplication1.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(IFormFile resume)
        {
            ...
            // Check unique email and handle gracefully
            var existing = await _userManager.FindByEmailAsync(Input.Email);
            if (existing != null)
            {
                // Record audit for attempted registration with existing email
                _db.AuditLogs.Add(new AuditLog
                {
                    UserId = existing.Id,
                    Event = "RegisterAttemptExistingEmail",
                    IpAddress = HttpContext.Connection.RemoteIpAddress?.ToString(),
                    Details = $"Attempted registration for existing email {Input.Email}",
                    OccurredAt = DateTime.UtcNow
                });
                await _db.SaveChangesAsync();

                ModelState.AddModelError("Input.Email", "An account with this email already exists. If you forgot your password, use Forgot Password.");
                return Page();
            }
